...
var zeroBlock = make([]byte, defaultConfig.BlockSize)

func TestFormat(t *testing.T) {
	dev := NewMemoryDevice(defaultConfig)
	...
	t.Run("InvalidSuperblocks", func(t *testing.T) {
		...
		if err := dev.ProgramBlock(0, 0, zeroBlock); err != nil {
			t.Fatal(err)
		}
		if err := dev.ProgramBlock(1, 0, zeroBlock); err != nil {
			t.Fatal(err)
		}
		if err := fs.Format(); err == nil {
...
func createTestFS(t *testing.T, config Config) (*LFS, BlockDevice, func()) {
	// create/format/mount the filesystem
	bd := NewMemoryDevice(config)
	fs := New(config, bd)
	if err := fs.Format(); err != nil {
